Shader "FruitFlyJoust/FlyBodyDepth"
{
    Properties
    {
        _BaseColor ("Biomodel color", Color) = (0.55,0.28,0.08,1)
    ...
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float texture=lerp(1-_TextureStrength,1+_TextureStrength*.42,broad*.65+fine*.35);
            float facing=saturate(dot(normalize(IN.viewDir),n));
            float rim=pow(1-facing,3)*.055;
            o.Albedo=_BaseColor.rgb*underside*texture;
            o.Metallic=.03;
            o.Smoothness=_Smoothness;
            o.Occlusion=saturate(underside*(.86+.14*broad));
            o.Emission=_BaseColor.rgb*rim;
            o.Alpha=1;
        }
        ENDCG
    }
