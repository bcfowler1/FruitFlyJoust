...
class LandingLegIK:
    def __init__(self,physics,walker,anchor,posture_gain=0,capsule_clearance=False):
        self.physics=physics;self.anchor=anchor;self.legs=[]
        self.capsule_clearance=capsule_clearance
        ...
        self.swing_goal=None;self.swing_posture_gain=0
    ...
    def update(self):
        ...
        for geom,entries,offset in self.legs:
            ...
            if self.capsule_clearance:
                # Capsule local Z is its longitudinal axis. The lowest point
                # includes the projected half-length, not only the radius.
                if int(m.geom_type[geom])==int(mujoco.mjtGeom.mjGEOM_CAPSULE):
                    goal[2]+=float(m.geom_size[geom,1])*abs(float(d.geom_xmat[geom].reshape(3,3)[2,2]))
            ...
            gain=self.swing_posture_gain if self.swing_goal is not None and self.swing_goal[0]==geom else self.posture_gain
            if gain:
                ...
                correction+=gain*(np.eye(len(entries))-inverse@jac)@posture
    ...
    def lower_unloaded_feet(self,supported):
        """Press unsupported claws toward the floor, preserving loaded controls."""
        m=self.physics.model;d=self.physics.data
        for geom,entries,_ in self.legs:
            if m.id2name(geom,'geom') in supported:continue
            clearance=float(m.geom_size[geom,0])
            if int(m.geom_type[geom])==int(mujoco.mjtGeom.mjGEOM_CAPSULE):
                clearance+=float(m.geom_size[geom,1])*abs(float(d.geom_xmat[geom].reshape(3,3)[2,2]))
            error=np.array([0.,0.,np.clip(clearance-.0005-d.geom_xpos[geom,2],-.02,.02)])
            mujoco.mj_jacGeom(m.ptr,d.ptr,self.jac,self.rot,geom)
