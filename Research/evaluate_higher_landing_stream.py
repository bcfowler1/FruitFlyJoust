"""Exercise the separately gated higher-landing stream, pause and replay."""
import argparse
...
def main():
    ...
    prefix='higher-landing-stream-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')
    ...
    input_port,output_port=55570,55571
    with socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as receiver,log.open('x',encoding='utf-8') as output:
        ...
        process=subprocess.Popen([sys.executable,str(root/'serve_higher_landing.py'),'--qualification',str(args.qualification.resolve()),
        ...
        mode='starting';frames=0;max_gap=0;completed=False;replayed=False;phases=set()
        try:
            while time.monotonic()-start<300:
                ...
                frame=json.loads(raw);frames+=1;phases.add(frame['episode_status'])
                assert frame['protocol']==1 and frame['neurons']==0 and frame['neural_time']==0
                assert 'engineering stance bridge' in frame['body_controller']
            ...
            assert max_gap<.035, 'Body-clock discontinuity leaked into stream'
            output.flush();trials=[]
            ...
            result.update(qualified=True,frames=frames,exact_paused_geometry_and_clock=True,
                ...
                explicit_replay_new_session=True,maximum_stream_time_gap=max_gap,observed_phases=sorted(phases),
                limitations='Engineering stance bridge; native walking policy; no connectome-derived limb controller or live rein qualification.')
