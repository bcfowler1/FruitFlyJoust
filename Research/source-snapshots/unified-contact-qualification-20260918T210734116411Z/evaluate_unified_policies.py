...
def main(launch_height=.15,output_name='unified-policy-evaluation.json',landing=False,upright_landing=True,contact_handover=False,stance_before_launch=False,leg_ik=False,claw_hold=False,resume_walking=False,seed=42,grip_ramp=.2,stance_ramp=.5,walking_only=False,recover_stance=False,hold_seconds=.3,hold_head=False,filter_settle_seconds=0,claw_release_seconds=0,command_slew_rate=0,walking_seconds=.5,match_resume_height=False,contact_guard=False,guard_leg_ik=False,resume_speed=.2,grip_min_up=-1,balanced_grip=False,body_recovery=False,replant_feet=False,grip_max_speed=float("inf"),touchdown_recovery=False,adaptive_grip=False,automatic_recovery=False,heading_leg_ik=False,loaded_stance_handover=False):
    ...
    if leg_ik:
        ...
        ik=LandingLegIK(physics,walker,anchor)
    ...
    result=dict(qualified=bool(rms<.15 and max(ground_contacts)==0 and max(attitude_errors)<.15),random_seed=seed,
    ...
    if landing:
        ...
        for step in range(20000):
            ...
            upright_blend=min(1.,step*.0002/.6) if upright_landing else 0.
            target_quaternion=(1-upright_blend)*flight_quaternion+upright_blend*np.array([1.,0,0,0])
            ref[:,3:7]=target_quaternion/np.linalg.norm(target_quaternion)
            ref[:,:3]=walked+np.array([0,0,launch_height*(1-progress)])
            action=policies['flight'](observation(flight.task,physics,True)).mean().numpy()[0]
            spec=flight.action_spec();flight.task.before_step(physics,np.clip(action,spec.minimum,spec.maximum).copy(),rng)
            for joint in leg_joints:
                actuator=walker.mjcf_model.find('actuator',joint.name)
            ...
            if hold_started is not None:
                ...
                physics.data.ctrl[wing_controls]*=max(0.,1-elapsed_hold/grip_ramp)
                ...
                for joint in leg_joints:
                    actuator=walker.mjcf_model.find('actuator',joint.name)
                    if actuator is not None:
                        current=float(physics.bind(actuator).ctrl)
                        physics.bind(actuator).ctrl=(1-stance_blend)*current+stance_blend*(loaded_stance_targets[joint.name] if loaded_stance_handover else initial_leg_angles[joint.name])
        ...
        result['landing']=dict(qualified=bool(finite and stable_time>=.1 and handover_ready),contact_transition_completed=handover_ready,stable_seconds=stable_time,upright_reference=upright_landing,stance_before_launch=stance_before_launch,
            loaded_stance_handover=loaded_stance_handover,loaded_stance_targets=loaded_stance_targets if loaded_stance_handover else None,heading_relative_leg_ik=heading_leg_ik,contact_handover=contact_handover,walking_policy_handover_time=walk_started,physical_leg_ik=leg_ik,
        ...
        if resume_walking and result['landing']['qualified']:
            ...
            if automatic_recovery:
                ...
                native_prediction=predict_native_handoff(walk,physics,policies['walk'],observation,rng,anchor,
                    wing_controls,claw_actuators,walking_dynamics,resume_speed,filter_settle_seconds,claw_release_seconds)
            ...
            if body_recovery:
                ik.begin_body_recovery(float(walk.task._ref_qpos[0,2]))
            ...
            if replant_feet and hold_ok:
                ...
                for geom,entries,offset in ik.legs:
                    ...
                    if not suffix.startswith('T1_'):continue
                    ...
                    target_foot[2]=float(physics.model.geom_size[geom,0])+.001
                    foot_safe=[]
            ...
            walk.task._ref_qpos[:,3:7]=[np.cos(yaw/2),0,0,np.sin(yaw/2)]
            ...
            for resume_index in range(round(walking_seconds/.002)):
                ...
                blend=min(1.,resume_index*.002/.25)
                if contact_guard:
            ...
            result['walking_resume']=dict(native_handoff_prediction=native_prediction,recovery_requested=requested_body_recovery,diagnostic_foot_replant=replant_feet,foot_replant_rows=replant_rows,
                ...
                command_blend_seconds=.25,claw_release_seconds=claw_release_seconds,claw_release='engineering grip ramp then native policy' if claw_release_seconds else 'native walking policy',
                initial_root=resume_position[:7].tolist(),reference_heading_rad=yaw,rows=resume_rows,
