...
def predict_native_handoff(walk, physics, policy, observe, rng, anchor, wings,
                           claws, dynamics, speed, settle_seconds, release_seconds):
    ...
    try:
        ...
        task._ref_qpos[:,3:7]=[np.cos(yaw/2),0,0,np.sin(yaw/2)]
        ...
        return dict(qualified=qualified,completed_seconds=len(up)*.002,minimum_up=min(up),
                    ...
                    selection='native handoff' if qualified else 'physical posture/front-foot recovery',
