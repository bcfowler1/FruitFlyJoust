...
def main(launch_height=.15,output_name='unified-policy-evaluation.json',landing=False,upright_landing=True,contact_handover=False,stance_before_launch=False,leg_ik=False,claw_hold=False,resume_walking=False,seed=42,grip_ramp=.2,stance_ramp=.5,walking_only=False,recover_stance=False,hold_seconds=.3,hold_head=False,filter_settle_seconds=0,claw_release_seconds=0,command_slew_rate=0,walking_seconds=.5,match_resume_height=False,contact_guard=False,guard_leg_ik=False,resume_speed=.2,grip_min_up=-1,balanced_grip=False,body_recovery=False,replant_feet=False,grip_max_speed=float("inf"),touchdown_recovery=False,adaptive_grip=False,automatic_recovery=False,heading_leg_ik=False,loaded_stance_handover=False,landing_pitch_degrees=None,capsule_clearance=False,synchronize_wings=False,complete_contacts=False,replant_all_feet=False,measured_stance_recovery=False,swing_posture_gain=0,restore_loaded_controls=False,walking_blend_seconds=.25):
    ...
    if walking_only:
        ...
        Path(__file__).with_name(output_name).write_text(json.dumps(result,indent=2,default=lambda value:value.tolist() if hasattr(value,'tolist') else float(value)));print(json.dumps(result),flush=True)
        return
    ...
    ik=None
    if leg_ik:
        from loaded_landing_reach_ik import LoadedLandingReachIK as LandingLegIK
    ...
    reference_frames=80000 if landing else 15000
    ...
    leg_controls=[physics.model.name2id(walker.mjcf_model.find('actuator',joint.name).full_identifier,'actuator')
        for joint in leg_joints if walker.mjcf_model.find('actuator',joint.name) is not None]
    ...
    if landing:
        ...
        peak_stable_time=0.;support_loss_trace=[];first_support_loss=None
        flight_quaternion=ref[0,3:7].copy()
        walk_started=None;walk_counter=0;wing_at_contact=None;walking_ctrl=None
        hold_started=None;grip_wing_angles=None;grip_start_state=None;contact_completion_steps=0;free_contact_targets={}
        reached_claws=set();reached_targets={};first_reach_time=None;motor_feedback_report=None;motor_feedback_updates=0;motor_feedback_controls={}
        ...
        for step in range(40000):
            ...
            if ik and step*.0002>=approach_seconds:
                ...
                if reached_claws:
                    ...
                    for geom,entries,_ in ik.legs:
                        if physics.model.id2name(geom,'geom') in feet:
                            for _,_,aid,_ in entries:
                                if aid in reached_targets:physics.data.ctrl[aid]=reached_targets[aid]
            if reached_claws and hold_started is None:
                from grasp_motor_feedback import support_targets
                # While wings still support weight, contact motors damp the
                # body and retain actual grip; defer folding until front/rear support.
                if step%10==0:
                    motor_feedback_controls,motor_feedback_report=support_targets(physics,ik,anchor,feet,ik.initial_height,.3)
                    motor_feedback_updates+=1
                for aid,target in motor_feedback_controls.items():physics.data.ctrl[aid]=target
            ...
            if claw_hold and (not balanced_grip or support_balanced) and hold_started is None and contacts>=4 and support_balanced and body_contacts==0 and grip_attitude_ok and np.linalg.norm(physics.data.qvel[:3])<=grip_max_speed:
                ...
                grip_wing_angles=np.array([float(physics.data.qpos[physics.model.jnt_qposadr[physics.model.actuator_trnid[aid,0]]]) for aid in wing_controls])
                resting_wing_angles=np.array([float(handover_position[physics.model.jnt_qposadr[physics.model.actuator_trnid[aid,0]]]) for aid in wing_controls])
            ...
            if hold_started is not None:
                ...
                wing_fold=min(1.,elapsed_hold/grip_ramp)
                desired_wing_angles=(1-wing_fold)*grip_wing_angles+wing_fold*resting_wing_angles
                actual_wing_angles=np.array([float(physics.data.qpos[physics.model.jnt_qposadr[physics.model.actuator_trnid[aid,0]]]) for aid in wing_controls])
                # Native FlyBody wing actuators are torque controls. Match the
                # published before_step conversion of target angles to errors.
                physics.data.ctrl[wing_controls]=desired_wing_angles-actual_wing_angles
                stance_blend=0. if touchdown_recovery else min(1.,max(0.,(elapsed_hold-(0 if loaded_stance_handover else grip_ramp))/stance_ramp))
                if step%10==0 or not motor_feedback_controls:
                    from grasp_motor_feedback import support_targets
                    motor_feedback_controls,motor_feedback_report=support_targets(physics,ik,anchor,feet,ik.initial_height,min(1.,elapsed_hold/grip_ramp))
                    motor_feedback_updates+=1
                for aid,target in motor_feedback_controls.items():physics.data.ctrl[aid]=target
            ...
            stable_now=contacts>=4 and body_contacts==0 and up>.9 and speed<.5
            previous_stable=stable_time
            stable_time=stable_time+.0002 if stable_now else 0.
            peak_stable_time=max(peak_stable_time,stable_time)
            if peak_stable_time>=.1 and (stable_now or previous_stable>0):
                support_loss_trace.append(dict(time=float(physics.data.time),contacts=sorted(feet),
                    body_contacts=body_contacts,up_cosine=up,speed_cm_s=speed,
                    support_feedback=motor_feedback_report,
                    leg_ctrl=[float(physics.data.ctrl[i]) for i in leg_controls],
                    leg_force=[float(physics.data.actuator_force[i]) for i in leg_controls]))
                support_loss_trace=support_loss_trace[-20:]
            if peak_stable_time>=.1 and not stable_now and first_support_loss is None:
                first_support_loss=support_loss_trace[-1] if support_loss_trace else dict(time=float(physics.data.time))
                break
            ...
            if not finite or (stable_time>=hold_seconds+.1 and handover_ready):break
        result['landing']=dict(engineering_contact_motor_feedback=True,motor_feedback_updates=motor_feedback_updates,motor_feedback=motor_feedback_report,wing_transition='native angle-error torque conversion; measured grip to measured folded wing targets',individual_contact_grip=True,loaded_stance_transfer_supported_legs_only=True,front_reach_nearest_hip_floor_point=True,reacquire_lost_contacts=True,preserve_front_xyz_reach=True,wing_shutdown_requires_front_rear_four_claw_support=False,first_reach_time=first_reach_time,reached_claws=sorted(reached_claws),approach_seconds=approach_seconds,approach_legs_tucked=True,qualified=bool(finite and stable_time>=.1 and handover_ready),contact_transition_completed=handover_ready,stable_seconds=stable_time,landing_pitch_reference_degrees=landing_pitch_degrees,upright_reference=upright_landing,stance_before_launch=stance_before_launch,
            ...
            peak_stable_seconds=peak_stable_time,first_support_loss=first_support_loss,
            support_loss_trace=support_loss_trace,
        ...
        if resume_walking and result['landing']['qualified']:
            ...
            # The same actual landing loop just maintained all strict hold
            # limits continuously for hold_seconds + .1, sampled every .2ms.
            held_contacts=[contacts];held_up=[up];held_speed=[speed]
            hold_ok=bool(stable_time>=hold_seconds+.1 and contacts>=4 and body_contacts==0 and up>.9 and speed<.5)
            ...
            demonstrated=json.loads(Path(__file__).with_name('unified-contact-qualification-20260918T213219801072Z-seed42.json').read_text())['walking_resume']['landed_state']['joints']
            target_physics=physics.copy(share_model=True)
            matrix=physics.data.xmat[anchor].reshape(3,3);yaw_target=float(np.arctan2(matrix[1,0],matrix[0,0]));c,sn=np.cos(yaw_target),np.sin(yaw_target)
            level_rotation=np.array([[c,-sn,0],[sn,c,0],[0,0,1.]])
            target_physics.data.qpos[3:7]=np.array([np.cos(yaw_target/2),0,0,np.sin(yaw_target/2)])
            target_physics.data.qpos[2]=float(walk.task._ref_qpos[0,2])
            for joint_name,state in demonstrated.items():
                try:jid=target_physics.model.name2id('walker/'+joint_name,'joint')
                except Exception:continue
                target_physics.data.qpos[target_physics.model.jnt_qposadr[jid]]=float(state['position'][0])
            mujoco.mj_forward(target_physics.model.ptr,target_physics.data.ptr)
            target_locals={geom:level_rotation.T@(target_physics.data.geom_xpos[geom]-target_physics.data.xpos[anchor]) for geom,_,_ in ik.legs}
            manifold_rows=[]
            for segment in ('T1','T3','T2'):
                for side in ('left','right'):
                    suffix=segment+'_'+side
                    geom,entries,_=next(item for item in ik.legs if physics.model.id2name(item[0],'geom').endswith(suffix+'_collision'))
                    adhesion=next(a for a in claw_actuators if physics.model.id2name(a,'actuator').endswith(suffix))
                    start=physics.data.geom_xpos[geom].copy();goal=physics.data.xpos[anchor]+level_rotation@target_locals[geom]
                    goal[2]=float(physics.model.geom_size[geom,0])+.001
                    planar=goal[:2]-start[:2];distance=float(np.linalg.norm(planar))
                    if distance>.04:goal[:2]=start[:2]+planar*(.04/distance)
                    safe=True
                    for transition_step in range(500):
                        phase=(transition_step+1)/500.;smooth=phase*phase*(3-2*phase)
                        swing=start*(1-smooth)+goal*smooth;swing[2]+=.02*np.sin(np.pi*phase)
                        feet,other=claw_contacts(physics.model,physics.data)
                        support,feedback=support_targets(physics,ik,anchor,feet-{physics.model.id2name(geom,'geom')},ik.initial_height,1.)
                        for aid,target in support.items():physics.data.ctrl[aid]=target
                        ik.swing_goal=(geom,swing);ik.update()
                        for _,_,aid,_ in entries:physics.data.ctrl[aid]=ik.targets[aid]
                        physics.data.ctrl[claw_actuators]=1;physics.data.ctrl[adhesion]=0;physics.data.ctrl[wing_controls]=0
                        physics.step(40)
                        feet,other=claw_contacts(physics.model,physics.data);up_now=float(physics.data.xmat[anchor].reshape(3,3)[2,2])
                        safe=len(feet)>=4 and other==0 and up_now>.9
                        if not safe:break
                    ik.swing_goal=None;physics.data.ctrl[adhesion]=1
                    for _ in range(100):
                        feet,other=claw_contacts(physics.model,physics.data);support,feedback=support_targets(physics,ik,anchor,feet,ik.initial_height,1.)
                        for aid,target in support.items():physics.data.ctrl[aid]=target
                        if physics.model.id2name(geom,'geom') not in feet:
                            from landing_leg_ik import LandingLegIK as BaseLandingLegIK
                            BaseLandingLegIK.lower_unloaded_feet(ik,feet)
                        physics.data.ctrl[claw_actuators]=1;physics.data.ctrl[wing_controls]=0;physics.step(40)
                    feet,other=claw_contacts(physics.model,physics.data);up_now=float(physics.data.xmat[anchor].reshape(3,3)[2,2])
                    manifold_rows.append(dict(leg=suffix,completed_steps=transition_step+1,safe=safe,claws=len(feet),other_contacts=other,up=up_now,target_error_cm=float(np.linalg.norm(physics.data.geom_xpos[geom]-goal))))
                    hold_ok=bool(hold_ok and safe and len(feet)>=4 and physics.model.id2name(geom,'geom') in feet and other==0 and up_now>.9)
                    if not hold_ok:break
                if not hold_ok:break
            print(json.dumps(dict(stage='cartesian_walking_manifold_transition',qualified=hold_ok,rows=manifold_rows)),flush=True)
            ...
            if filter_settle_seconds:
                ...
                for settle_index in range(round(filter_settle_seconds/.002)):
                    feet,other=claw_contacts(physics.model,physics.data)
                    settle_targets,settle_feedback=support_targets(physics,ik,anchor,feet,ik.initial_height,1.)
                    for aid,target in settle_targets.items():physics.data.ctrl[aid]=target
                    physics.data.ctrl[claw_actuators]=1
                    physics.step(40)
                    feet,other=claw_contacts(physics.model,physics.data)
    ...
    Path(__file__).with_name(output_name).write_text(json.dumps(result,indent=2,default=lambda value:value.tolist() if hasattr(value,'tolist') else float(value)))
