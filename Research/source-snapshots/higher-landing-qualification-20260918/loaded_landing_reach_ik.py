...
class LoadedLandingReachIK(LandingLegIK):
    def lower_unloaded_feet(self,supported):
        # Retain the front feet's full surface-directed XYZ command rather
        # than replacing it with the generic vertical-only pressure command.
        saved={aid:float(self.physics.data.ctrl[aid]) for geom,entries,_ in self.legs
            if 'T1_' in self.physics.model.id2name(geom,'geom') for _,_,aid,_ in entries}
        super().lower_unloaded_feet(supported)
        if self.body_recovery is None:
            for aid,target in saved.items():self.physics.data.ctrl[aid]=target
    ...
    def update(self):
        ...
        for geom,entries,_ in self.legs:
            ...
            for (_,qpos,aid,jid),change in zip(entries,delta):
                ...
                target+=float(d.qfrc_bias[dof]-spring)/gain
