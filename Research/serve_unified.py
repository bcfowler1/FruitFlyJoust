"""Stream a qualified automated same-body floor loop; no live rein/brain mapping.
...
def qualification(path):
    ...
    if not result.get('qualified') or len(set(result.get('seeds_requested',[])))<5 or not all(row.get('qualified') for row in result.get('experiments',[])):
        raise RuntimeError('Requires a completed five-initialization qualification')
    parameters=result['controller_parameters']
    expected=dict(wing_cut_seconds=.2,stance_blend_seconds=.2,
        walking_blend_seconds=0,claw_release_seconds=0,synchronized_wing_phase=True,
        unsupported_contact_completion=True,loaded_stance_handover=True)
    if any(parameters.get(key)!=value for key,value in expected.items()):
        raise RuntimeError('Qualification does not match this lab controller')
    for name,digest in result['source_sha256'].items():
        if hashlib.sha256((ROOT/name).read_bytes()).hexdigest()!=digest:
            raise RuntimeError('Source changed since qualification: '+name)
    return result
...
class Stream:
    ...
    def frame(self):
        ...
        packet=dict(protocol=1,session=self.session,seq=self.seq,sim_time=float(d.time),
            ...
            body_controller='Automated trained-policy floor qualification; no connectome or live rein mapping',
            rider_cue_mapping='Camera/pause/replay only; automated body commands')
...
def main():
    ...
    if args.seed not in qualified['seeds_requested']:parser.error('Seed was not qualified')
    step=mjcf.Physics.step;apply=evaluation.RiderLoad.apply
        ...
        while True:
            ...
            def stage_print(*values,**kwargs):
                if values and isinstance(values[0],str):
                    ...
                    if event.get('stage')=='native_handoff_prediction' and event.get('result',{}).get('qualified'):
                        stream.phase='walking handoff'
                        stream.walking_start=float(stream.physics.data.time)+.1
                print(*values,**kwargs)
            report='unified-live-'+stream.session+'.json'
            try:
                mjcf.Physics.step=observed_step;evaluation.RiderLoad.apply=loaded;evaluation.print=stage_print
                evaluation.main(output_name=report,landing=True,upright_landing=False,
                    stance_before_launch=True,leg_ik=True,claw_hold=True,resume_walking=True,
                    seed=args.seed,grip_ramp=.2,stance_ramp=.2,hold_seconds=2,
                    filter_settle_seconds=.1,claw_release_seconds=0,walking_seconds=6,
                    resume_speed=1,grip_max_speed=2,body_recovery=True,replant_feet=True,
                    adaptive_grip=True,automatic_recovery=True,loaded_stance_handover=True,
                    synchronize_wings=True,complete_contacts=True,walking_blend_seconds=0)
                ...
                mjcf.Physics.step=step;evaluation.RiderLoad.apply=apply
