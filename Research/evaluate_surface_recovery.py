...
def main(launch_height=.15,output_name='unified-policy-evaluation.json',landing=False,upright_landing=True,contact_handover=False,stance_before_launch=False,leg_ik=False,claw_hold=False,resume_walking=False,seed=42,grip_ramp=.2,stance_ramp=.5,walking_only=False,recover_stance=False,hold_seconds=.3,hold_head=False,filter_settle_seconds=0,claw_release_seconds=0,command_slew_rate=0,walking_seconds=.5,match_resume_height=False,contact_guard=False,guard_leg_ik=False,resume_speed=.2,grip_min_up=-1,balanced_grip=False,body_recovery=False,replant_feet=False,grip_max_speed=float("inf"),touchdown_recovery=False,adaptive_grip=False,automatic_recovery=False,heading_leg_ik=False,loaded_stance_handover=False,landing_pitch_degrees=None,capsule_clearance=False,synchronize_wings=False,complete_contacts=False,replant_all_feet=False,measured_stance_recovery=False,swing_posture_gain=0,restore_loaded_controls=False,walking_blend_seconds=.25):
    ...
    if landing:
        ...
        reached_claws=set();reached_targets={};first_reach_time=None;support_recovery_started=None
        ...
        for step in range(20000):
            ...
            if ik and step*.0002>=approach_seconds:
                if step%10==0:
                    ...
                    ik.update()
                ik.apply()
                if reached_claws and support_recovery_started is None:
            ...
            if hold_started is not None:
                ...
                physics.data.ctrl[wing_controls]=(1-wing_fold)*grip_wing_angles+wing_fold*resting_wing_angles
                if support_recovery_started is None and elapsed_hold>=max(grip_ramp,stance_ramp)+.05:
                    ik.begin_body_recovery(float(walk.task._ref_qpos[0,2]),gain=.1,maximum_step=.001)
                    # Free claws must reach the actual surface, not retain their
                    # airborne height when forming an upright body target.
                    matrix=physics.data.xmat[anchor].reshape(3,3)
                    yaw=float(np.arctan2(matrix[1,0],matrix[0,0]));c,sine=np.cos(yaw),np.sin(yaw)
                    level=np.array([[c,-sine,0],[sine,c,0],[0,0,1]])
                    origin=physics.data.xpos[anchor].copy();origin[2]=float(walk.task._ref_qpos[0,2])
                    for geom,entries,_ in ik.legs:
                        goal=physics.data.geom_xpos[geom].copy()
                        if physics.model.id2name(geom,'geom') not in feet:
                            clearance=float(physics.model.geom_size[geom,0])
                            if int(physics.model.geom_type[geom])==int(mujoco.mjtGeom.mjGEOM_CAPSULE):clearance+=float(physics.model.geom_size[geom,1])*abs(float(physics.data.geom_xmat[geom].reshape(3,3)[2,2]))
                            goal[2]=clearance-.0005
                        ik.body_recovery[geom]=level.T@(goal-origin)
                    support_recovery_started=float(physics.data.time)
                ...
                for joint in leg_joints:
                    actuator=walker.mjcf_model.find('actuator',joint.name)
                    if actuator is not None and support_recovery_started is None:
            ...
            if step%500==0:landing_rows.append(dict(time=float(physics.data.time),contacts=contacts,body_contacts=body_contacts,up_cosine=up,speed_cm_s=speed,thorax_height_cm=float(physics.data.xpos[anchor,2]),claws=sorted(feet)))
